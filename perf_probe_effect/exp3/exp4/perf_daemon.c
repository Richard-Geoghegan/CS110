...
static int64_t get_monotonic_to_real_offset_ns(void) {
...
int main() {
    ...
    pe.sample_freq = 4000;       // 4 kHz (thesis default)
    pe.freq = 1;                 // Required when using sample_freq
    pe.sample_type = PERF_SAMPLE_IP | PERF_SAMPLE_TIME | PERF_SAMPLE_TID;
    pe.disabled = 1;
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    pe.wakeup_events = 20;       // Batch: wake after 20 samples (reduces overhead)

    int fd = perf_event_open(&pe, 0, -1, -1, 0);
    if (fd == -1) { perror("perf_event_open"); exit(EXIT_FAILURE); }

    long page_size = sysconf(_SC_PAGESIZE);
    int data_pages = 4;          // 16KB ring buffer
    ...
    // Setup async wakeup via epoll (replaces busy poll/SIGIO)
    ...
    int64_t time_offset = get_monotonic_to_real_offset_ns();
    uint64_t tail = header->data_tail;
    size_t data_size = data_pages * page_size;
    uint8_t *data_area = (uint8_t *)header + page_size;
    int exported = 0;

    printf("🟢 Async PMU daemon running (Ctrl+C to stop)\n");

    while (running) {
        struct epoll_event events[1];
        int nfds = epoll_wait(epoll_fd, events, 1, 1000); // 1s timeout

        if (nfds < 0) { perror("epoll_wait"); break; }
        if (nfds == 0) continue; // Timeout: no new samples

        // Kernel signaled ring buffer is ready
        ...
        while (tail < head) {
            ...
            if (hdr->type == PERF_RECORD_SAMPLE) {
                ...
                // Parse in kernel ABI order: IP -> TID(pid+tid) -> TIME
                if (pe.sample_type & PERF_SAMPLE_IP){ 
                    ip = *(uint64_t *)payload; payload += 8; 
                }
                if (pe.sample_type & PERF_SAMPLE_TID){ 
                    pid = *(uint32_t *)payload; payload += 4; 
                    tid = *(uint32_t *)payload; payload += 4; 
                }
                ...
                // Emit OpenTelemetry-compatible JSON 
                // span (one per line for easy piping)
                printf("{\"traceId\":\"%016lx%016lx\",\"spanId\":\"%016lx\",\"startTimeUnixNano\":\"%ld\",\"endTimeUnixNano\":\"%ld\",\"attributes\":{\"ip\":\"0x%lx\",\"pid\":%u,\"tid\":%u,\"counter\":\"cpu-cycles\"}}\n",
        ...
        header->data_tail = tail; // Advance consumer pointer
    }

    printf("\n📤 Exported %d spans to stdout\n", exported);
    printf("💡 Pipe to collector: ./perf_daemon | jq . > spans.json\n");
